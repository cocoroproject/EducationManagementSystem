...
public class StudentMenuView {
	...
	public void studentMenu() {
		...
		while(true) {
			...
			if(selectedMenu == 1) {
				new AlertView().alert("개인정보메뉴로 이동합니다.");
				Controllers.getStudentMainController().requestStudentInformationView();
			} else if(selectedMenu == 2) {
				new AlertView().alert("신청정보메뉴로 이동합니다.");
				
			} else if(selectedMenu == 3) {
				new AlertView().alert("수강정보메뉴로 이동합니다.");
			
			} else if(selectedMenu == 4) {
				new AlertView().alert("성적정보메뉴로 이동합니다.");
			
			} else if(selectedMenu == 5) {
				new AlertView().alert("학사공지메뉴로 이동합니다.");
		
			} else if(selectedMenu == 6) {
				new AlertView().alert("로그아웃 하셨습니다.");
